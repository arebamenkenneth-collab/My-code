...
def draw_heart(surface, cx, cy, size, alpha=255):
    heart_surf = pygame.Surface((size, size), pygame.SRCALPHA)
    s = size
    r = s // 4
    pygame.draw.circle(heart_surf, (255, 60, 100, alpha), (s // 4, s // 4), r)
    pygame.draw.circle(heart_surf, (255, 60, 100, alpha), (3 * s // 4, s // 4), r)
    points = [(0, s // 4), (s // 2, s - 2), (s, s // 4)]
    pygame.draw.polygon(heart_surf, (255, 60, 100, alpha), points)
    pygame.draw.circle(heart_surf, (255, 160, 180, min(alpha, 180)), (s // 4 + 2, s // 4 - 2), max(2, r // 2))
    surface.blit(heart_surf, (cx - s // 2, cy - s // 2))

def draw(player, stars, score, lives, high_score, explosions, shield_item, shield_timer, paused, heart_item):
    WIN.blit(BG, (0, 0))
    ...
    if heart_item:
        ...
        size = int(HEART_WIDTH + 6 * pulse)
        draw_heart(WIN, heart_item.centerx, heart_item.centery, size)
...
def main():
    while True:
        ...
        while run:
            ...
            if paused:
                draw(player, stars, score, lives, high_score, explosions, shield_item, shield_timer, paused, heart_item)
                continue
            ...
            if (heart_item is None and
                    heart_spawn_counter >= HEART_SPAWN_THRESHOLD and
                    lives < MAX_LIVES and
                    random.random() < HEART_SPAWN_CHANCE):
                x = random.randint(0, WIDTH - HEART_WIDTH)
                heart_item = pygame.Rect(x, -HEART_HEIGHT, HEART_WIDTH, HEART_HEIGHT)
                heart_spawn_counter = 0
            elif heart_item is None and heart_spawn_counter >= HEART_SPAWN_THRESHOLD:
                heart_spawn_counter = 0
            ...
            if heart_item:
                heart_item.y += max(2, star_vel - 5)
                if heart_item.y > HEIGHT:
                    heart_item = None
                elif heart_item.colliderect(player):
                    heart_item = None
                    lives = min(lives + 1, MAX_LIVES)
                    explosions.append({
                        "pos": (player.centerx, player.centery - 20),
                        "radius": 6,
                        "color": (255, 80, 120),
                        "life": 25
                    })
